...
my_vec_bool
"Four values of my_vec_double are greater than zero"
```
...
data <- seq(1,100)
datasquared <- data^2
dataInput <- c(data,datasquared)
matrix(data = dataInput, nrow=2, ncol=100, byrow = TRUE)
...
With smaller matricies, the col-wise scans are faster, which makes sense since matrices in R are stored by column. However, as the matrix get bigger, the row wise scans are faster than the column-wise ones. 
